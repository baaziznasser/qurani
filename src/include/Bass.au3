...
if @AutoItX64 then
$sBassDLL = StringReplace($sBassDLL, ".dll", "_x64.dll")
endIf
...
; Description ...:
; Syntax.........: _BASS_GetEAXParameters($env, $vol, $decay, $damp)
; Parameters ....:  -	$env 	-	The EAX environment...
...
; Syntax.........: _BASS_MusicLoad($mem, $file, $offset, $offsethigh, $length, $flags, $freq)
...
; Syntax.........: _BASS_SampleLoad($mem, $file, $offset, $offsethigh, $length, $max, $flags)
...
; Syntax.........: _BASS_SampleGetData($handle)
; Parameters ....:  -	$handle		-	The sample handle
...
Func _BASS_SampleGetInfo($handle)
	...
	Local $BASS_ret_ = DllCall($_ghBassDll, "int", "BASS_SampleGetInfo", "dword", $handle, "ptr", $BASS_SAMPLE_INFO_S)
	If @error Then Return SetError(1,1,0)
	If $BASS_ret_[0] = 0 Then Return SetError(_BASS_ErrorGetCode(),0,0)
		For $i = 1 To 17
...
; #FUNCTION# ====================================================================================================================
; Name...........: _BASS_SampleSetInfo($handle, $info)
; Description ...:
; Syntax.........:
; Parameters ....:  -	$handle		-	Handle to a sample
...
; Example .......;
; ===============================================================================================================================
Func _BASS_StreamCreate($freq, $chans, $flags, $proc, $pUser)
	Local $proc_s=-1
	If IsString($proc) Then
		$proc_s = DllCallbackRegister ($proc, "ptr", "dword;char[255];dword;char[255]")
...
; Description ...: Creates a sample stream from an MP3, MP2, MP1, OGG, WAV, AIFF or plugin supported file on the internet,
;				   optionally receiving the downloaded data in a callback function.
...
; #FUNCTION# ====================================================================================================================
; Name...........: _BASS_StreamPutData($handle, $buffer, $length)
...
; Syntax.........: _BASS_RecordGetInputName($inputn)
; Parameters ....:  -	$inputn 	-	The input to get the description of... 0 = first, -1 = master.
...
; Parameters ....:  -	$inputn		-	The input to adjust the settings of... 0 = first, -1 = master.
...
; Example .......;
; ===============================================================================================================================
Func _BASS_RecordSetInput($inputn, $flags, $volume)
	Local $BASS_ret_ = DllCall($_ghBassDll, "int", "BASS_RecordSetInput", "int", $inputn, "DWORD", $flags, "float", $volume)
	If @error Then Return SetError(1,1,0)
	If $BASS_ret_[0] = 0 Then Return SetError(_BASS_ErrorGetCode(),0,0)
	Return $BASS_ret_[0]
...
; Parameters ....:  -	$inputn		-	The input to get the settings of... 0 = first, -1 = master.
...
; Example .......;
; ===============================================================================================================================
Func _BASS_RecordGetInput($inputn)
	Local $aRet[2]
	Local $BASS_ret_ = DllCall($_ghBassDll, "dword", "BASS_RecordGetInput", "int", $inputn, "float*", 0)
	If @error Then Return SetError(1,1,0)
	If $BASS_ret_[0] = 0 Then Return SetError(_BASS_ErrorGetCode(),0,0)
		$aRet[0] = $BASS_ret_[0]
		$aRet[1] = $BASS_ret_[2]
...
; Example .......;
; ===============================================================================================================================
Func _BASS_RecordStart($freq, $chans, $flags, $proc = "", $pUser = 0)
	Local $dcProc = -1
	If IsString($proc) Then
		$dcProc = DllCallbackRegister ($proc, "int", "dword;ptr;dword;ptr;")
		$proc = DllStructGetPtr($dcProc)
	EndIf
...
; #FUNCTION# ====================================================================================================================
; Name...........: _BASS_ChannelIsActive($handle)
...
; #FUNCTION# ====================================================================================================================
; Name...........: _BASS_ChannelSet3DAttributes($handle, $mode, $min, $max, $iangle, $oangle, $outvol)
...
; Syntax.........: _BASS_ChannelSet3DPosition($handle, $pos, $orient, $vel)
; Parameters ....:  -	$handle		-	Handle The channel handle...
;							-	HCHANNEL, HMUSIC, HSTREAM, HRECORD or HSAMPLE handles accepted.
...
Func _BASS_ChannelGet3DPosition($handle)
	...
	Local $BASS_ret_ = DllCall($_ghBassDll, "int", "BASS_ChannelSet3DPosition", "DWORD", $handle, "PTR", DllStructGetPtr($s1), "PTR", DllStructGetPtr($s2), "PTR", DllStructGetPtr($s3))
	If @error Then Return SetError(1,1,0)
	If $BASS_ret_[0] = 0 Then Return SetError(_BASS_ErrorGetCode(),0,0)
		For $i = 0 To 2
...
;				   Can also be used with a recording channel.
; Syntax.........: _BASS_ChannelGetData($handle, $length)
; Parameters ....:  -	$handle		-	Handle The channel handle...
;										-	HCHANNEL, HMUSIC, HSTREAM, HRECORD or HSAMPLE handles accepted.
...
; Syntax.........: _BASS_FXSetParameters($fxhandle, $parameters)
; Parameters ....:
...
; Example .......;
; ===============================================================================================================================
Func _BASS_FXSetParameters($fxhandle, $parameters)
	Local $struct
...
; #FUNCTION# ====================================================================================================================
; Name...........: _BASS_SetEAXPreset($preset)
...
; Description ...: Helper function to set the volume of a channel.
; Syntax.........: _BASS_ChannelGetVolume($hChannel, $nVol)
; Parameters ....:  -   $hChannel	-	Handle to a channel
...
;===============================================================================
;
; Function Name:   __BASS_ReOrderULONGLONG
; Description::    INTERNAL USE
; Parameter(s):
; Requirement(s):
; Return Value(s):
; Author(s):       Prog@ndy
; Remarks:         Does nothing in beta 3.3.1.1 since it is fixed ;)
;
;===============================================================================
;
